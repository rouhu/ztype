...
ig.module('game.menus.settings', 'game.keyboard').requires('game.menus.base').defines(function () {
    ...
    MenuSettings = Menu.extend({
        ...
        init: function () {
            ...
            // Dynamically position items based on actual number of items
            for (var i = 0; i < this.items.length; i++) {
                this.items[i].y = (190 + i * 100) / this.scale;
            }

            // Adjust last item position to be near bottom
            if (this.items.length > 0) {
                this.items[this.items.length - 1].y = Math.max(ig.system.height - 120, (190 + (this.items.length - 1) * 100)) / this.scale;
            }
        },
        ...
        draw: function () {
            ...
            this.font.draw('MIN WORD DELAY', ig.system.width / 2 / s, 360 / s, ig.Font.ALIGN.CENTER);
            this.font.draw('WORD FALL SPEED', ig.system.width / 2 / s, 460 / s, ig.Font.ALIGN.CENTER);
            if (ig.ua.mobile) {
                this.font.draw('KEYBOARD  LAYOUT', ig.system.width / 2 / s, 560 / s, ig.Font.ALIGN.CENTER);
            }
            ctx.restore();
            var xs = this.width / 2;
            ctx.save();
