            ...
            if (loadCallback) {
                if (ig.ua.mobile) {
                    ...
                    var wrappedCallback = function () {
                        ...
                        clip.removeEventListener('canplaythrough', wrappedCallback, false);
                        clip.removeEventListener('error', wrappedCallback, false);
                        loadCallback(path, true, null);
                    };
                    timeoutId = setTimeout(wrappedCallback, 2000);
                    clip.addEventListener('canplaythrough', wrappedCallback, false);
                    clip.addEventListener('error', wrappedCallback, false);
                }
            }
            clip.preload = 'auto';
            clip.load();
    ...
    StatsView = ig.Class.extend({
        ...
        submit: function (data) {
            if (window.Cocoon || window.Ejecta) {
                var games = JSON.parse(ig.storageGet('stats') || '[]');
                if (games.length > 30) {
                    games.shift();
                }
                data.created = (Date.now() / 1000) | 0;
                games.push(data);
                ig.storageSet('stats', JSON.stringify(games));
                this.receivedData({
                    games: games
                });
            } else {
...
ig.module('game.menus.game-over').requires('game.menus.base', 'game.menus.interstitial', 'game.menus.stats').defines(function () {
    ...
    MenuItemFeedback = MenuItem.extend({
        getText: function () {
            return 'feedback?';
        },
        ok: function () {
            ig.game.openFeedbackForm();
        }
    });
...
ig.module('game.menus.settings', 'game.keyboard').requires('game.menus.base').defines(function () {
    MenuItemSoundVolume = MenuItem.extend({
        ...
        click: function () {
            ...
            ig.storageSet('soundVolume', ig.soundManager.volume);
        }
    });
    MenuItemMusicVolume = MenuItem.extend({
        ...
        click: function () {
            ...
            ig.storageSet('musicVolume', ig.music.volume);
        }
    });
    MenuItemWordDelay = MenuItem.extend({
        wide: true,
        getText: function () {
            var wordDelay = (ig.game.wordDelay || 0);
            return '- ' + wordDelay.toFixed(1) + 's +';
        },
        left: function () {
            ig.game.wordDelay = (ig.game.wordDelay || 0) - 0.5;
            if (ig.game.wordDelay < 0) {
                ig.game.wordDelay = 0;
            }
            ig.storageSet('wordDelay', ig.game.wordDelay);
        },
        right: function () {
            ig.game.wordDelay = (ig.game.wordDelay || 0) + 0.5;
            if (ig.game.wordDelay > 60) {
                ig.game.wordDelay = 60;
            }
            ig.storageSet('wordDelay', ig.game.wordDelay);
        },
        click: function () {
    ...
    MenuItemWordSpeed = MenuItem.extend({
        wide: true,
        getText: function () {
            var wordSpeed = (ig.game.wordSpeed || 1);
            return '- ' + wordSpeed.toFixed(1) + 'x +';
        },
        left: function () {
            ig.game.wordSpeed = (ig.game.wordSpeed || 1) - 0.1;
            if (ig.game.wordSpeed < 0.1) {
                ig.game.wordSpeed = 0.1;
            }
            ig.storageSet('wordSpeed', ig.game.wordSpeed);
        },
        right: function () {
            ig.game.wordSpeed = (ig.game.wordSpeed || 1) + 0.1;
            if (ig.game.wordSpeed > 3.0) {
                ig.game.wordSpeed = 3.0;
            }
            ig.storageSet('wordSpeed', ig.game.wordSpeed);
        },
        click: function () {
    ...
    MenuItemKeyboardLayout = MenuItem.extend({
        ...
        init: function () {
            this.layouts = [];
            this.current = 0;
            var selected = ig.storageGet('keyboardLayout');
            var i = 0;
        ...
        click: function () {
            ...
            ig.storageSet('keyboardLayout', this.layouts[this.current].key);
        }
    });
    ...
    MenuSettings = Menu.extend({
        ...
        init: function () {
            ...
            this.parent();
            this.y = ig.system.height / 6; // Reduced from /3 to /6

            // Dynamically position items with reduced spacing
            for (var i = 0; i < this.items.length; i++) {
                this.items[i].y = (140 + i * 80) / this.scale; // Reduced from 190 + i * 100
            }

            // Adjust last item position to be near bottom
            if (this.items.length > 0) {
                this.items[this.items.length - 1].y = Math.max(ig.system.height - 80, (140 + (this.items.length - 1) * 80)) / this.scale;
            }
        },
        itemClasses: [MenuItemSoundVolume, MenuItemMusicVolume, MenuItemWordDelay, MenuItemWordSpeed, MenuItemKeyboardLayout, MenuItemResume, MenuItemBack],
        draw: function () {
            this.width = ig.system.width / this.scale;
            var ctx = ig.system.context;
            this.parent();
            ...
            ctx.scale(s, s);
            ctx.globalAlpha = 0.5;
            this.font.draw('SOUND', ig.system.width / 2 / s, 120 / s, ig.Font.ALIGN.CENTER);
            this.font.draw('MUSIC', ig.system.width / 2 / s, 200 / s, ig.Font.ALIGN.CENTER);
            this.font.draw('MIN WORD DELAY', ig.system.width / 2 / s, 280 / s, ig.Font.ALIGN.CENTER);
            this.font.draw('WORD FALL SPEED', ig.system.width / 2 / s, 360 / s, ig.Font.ALIGN.CENTER);
            if (ig.ua.mobile) {
                this.font.draw('KEYBOARD  LAYOUT', ig.system.width / 2 / s, 440 / s, ig.Font.ALIGN.CENTER);
            }
            ctx.restore();
            var xs = this.width / 2;
            ctx.save();
            ...
            // Word delay item already has - + in its getText(), so don't draw them again
            if (ig.ua.mobile) {
                this.font.draw('<                                                 >', xs, this.items[3].y, ig.Font.ALIGN.CENTER);
            }
            ctx.restore();
        }
    });
...
ig.module('game.menus.title').requires('game.menus.base', 'game.menus.settings', 'game.menus.detailed-stats', 'game.ease').defines(function () {
    ...
    MenuTitle = Menu.extend({
        ...
        init: function () {
            ...
            if (window.Ejecta && !ig.storageGet('removeAds')) {
...
ig.module('game.entities.enemy').requires('impact.entity', 'impact.font', 'game.avenir-next', 'game.entities.explosion').defines(function () {
    EntityEnemy = ig.Entity.extend({
        ...
        init: function (x, y, settings) {
            ...
            if (ig.game.currentSpeech) {
                speechSynthesis.cancel();
            }
            var speech = new SpeechSynthesisUtterance(this.word);
            speech.lang = 'de-DE';
            speech.rate = 0.7;
            ig.game.currentSpeech = speech;
            speechSynthesis.speak(speech);
            console.log('Speaking: ' + this.word);
            ...
            // Apply word speed setting
            this.speed *= (ig.game.wordSpeed || 1);
        },
        getWordWithLength: function (l) {
            ...
            for (var i = 0; i < 20; i++) {
                ...
                if (!ig.game.targets[w.charAt(0).toLowerCase()] || !ig.game.targets[w.charAt(0).toLowerCase()].length) {
                    return w;
                }
            }
            return w;
        ...
        isHitBy: function (letter) {
            // Skip any leading spaces in the remaining word
            while (this.remainingWord.length > 0 && this.remainingWord.charAt(0) === ' ') {
                this.remainingWord = this.remainingWord.substr(1);
            }

            var expected = ig.game.translateUmlaut(this.remainingWord.charAt(0));
            if (!ig.game.textIsCaseSensitive) {
            ...
            if (expected == letter) {
                ...
                // Skip any spaces after the letter we just typed
                while (this.remainingWord.length > 0 && this.remainingWord.charAt(0) === ' ') {
                    this.remainingWord = this.remainingWord.substr(1);
                }
                ...
                if (this.remainingWord.length == 0) {
                    ig.game.currentTarget = null;
                    this.dead = true;
                    // Kill the entity immediately when all letters are typed
                    this.kill();
                }
                return true;
            } else {
...
ig.module('game.entities.emp').requires('impact.entity').defines(function () {
    ...
    window.EntityEMP = ig.Entity.extend({
        ...
        sound: new ig.Sound('media/sounds/emp.ogg'),
        init: function (x, y, settings) {
            this.parent(x, y, settings);
            this.idleTimer = new ig.Timer();
            this.sound.play();
...
ig.module('game.keyboard').requires('impact.font').defines(function () {
    ...
    ig.Keyboard.LAYOUT.QWERTZ = {
        ...
        map: [['q', 'w', 'e', 'r', 't', 'z', 'u', 'i', 'o', 'p'], ['a', 's', 'd', 'f', 'g', 'h', 'j', 'k', 'l'], ['y', 'x', 'c', 'v', 'b', 'n', 'm']]
    };
    ig.Keyboard.LAYOUT.AZERTY = {
        name: 'AZERTY (fr, be)',
        map: [['a', 'z', 'e', 'r', 't', 'w', 'u', 'i', 'o', 'p'], ['q', 's', 'd', 'f', 'g', 'h', 'j', 'k', 'l', 'm'], ['y', 'x', 'c', 'v', 'b', 'n']]
    };
    ig.Keyboard.LAYOUT.COLEMAK = {
        name: 'Colemak',
        map: [['q', 'w', 'f', 'p', 'g', 'j', 'l', 'u', 'y'], ['a', 'r', 's', 't', 'd', 'h', 'n', 'e', 'i', 'o'], ['z', 'x', 'c', 'v', 'b', 'k', 'm']]
    };
    ig.Keyboard.LAYOUT.DVORAK = {
        name: 'Dvorak',
        map: [['p', 'y', 'f', 'g', 'c', 'r', 'l'], ['a', 'o', 'e', 'u', 'i', 'd', 'h', 't', 'n', 's'], ['q', 'j', 'k', 'x', 'b', 'm', 'w', 'v', 'z', '']]
    };
});
...
ig.module('game.document-scanner').defines(function () {
    ...
    ig.DocumentScanner = ig.Class.extend({
        ...
        init: function (rootElement, enableNumbers, enablePunctionations) {
            ...
            this._reHasEnglishWords = /\b(the|be|to|of|and|that|have|with|you|this|say)\b/i;
            if (enableNumbers) {
                this._reHasWords = /\b\w{2,}\b/g;
            }
            if (enablePunctionations) {
                this._reHasWords = /[^\s]+/g;
            }
            ...
            console.log('Total fragments created:', this.fragments.length);
            for (var i = 0; i < this.fragments.length; i++) {
                console.log('Fragment ' + i + ':', this.fragments[i].text);
            }
        },
        ...
        splitIntoFragments: function (element, fragments, totalElements) {
            ...
            // If we have multiple elements (totalElements > 1), treat each element as ONE fragment
            // This is the case when custom.php creates separate divs for each sentence
            if (totalElements > 1) {
                // Just create one fragment from this element's text, no splitting
                fragments.push({
                    text: text,
                    element: element
                });
                console.log('Fragment created:', text);
                return;
            }

            // Original logic for single element with multiple sentences
            ...
            for (var l = 0; l < lines.length; l++) {
                ...
                if (line.length > 0) {
                    var lineSentences = line.replace(this._reSplitSentences, '$1%%ZT%%').split('%%ZT%%');
                    for (var s = 0; s < lineSentences.length; s++) {
                        if (lineSentences[s].trim().length > 0) {
                            sentences.push(lineSentences[s]);
                        }
                    }
                }
            }
...
ig.module('game.main').requires('impact.game', 'impact.font', 'game.menus.about', 'game.menus.game-over', 'game.menus.settings', 'game.menus.title', 'game.entities.enemy-missle', 'game.entities.enemy-mine', 'game.entities.enemy-destroyer', 'game.entities.enemy-oppressor', 'game.entities.player', 'game.keyboard', 'game.xhr', 'game.ease', 'plugins.silent-loader', 'plugins.rise-loader', 'game.document-scanner', 'game.words.en').defines(function () {
    ...
    ig.storageGet = function (key) {
        ...
        ;
    ig.storageSet = function (key, value) {
        try {
    ...
    ZType = ig.Game.extend({
        ...
        keyboard: null,
        textIsCaseSensitive: !!document.location.hash.match('cs'),
        textHasPunctuations: false,
        textHasNumbers: false,
        _screenShake: 0,
        wordlist: null,
        init: function () {
            var flags = parseInt(ig.queryParam('flags')) || window.ZTYPE_FLAGS;
            if (flags & ZType.FLAG.CASE_SENSITIVE) {
                this.textIsCaseSensitive = true;
            }
            if (flags & ZType.FLAG.PUNCTUATION) {
                this.textHasPunctuations = true;
            }
            if (flags & ZType.FLAG.NUMBERS) {
                this.textHasNumbers = true;
            }
            ...
            var soundVolume = ig.storageGet('soundVolume');
            var musicVolume = ig.storageGet('musicVolume');
            ...
            this.wordDelay = parseFloat(ig.storageGet('wordDelay')) || 0;
            this.wordSpeed = parseFloat(ig.storageGet('wordSpeed')) || 1;
            window.addEventListener('keypress', this.keypress.bind(this), false);
            window.addEventListener('keydown', this.keydown.bind(this), false);
            var layoutKey = ig.storageGet('keyboardLayout');
            ...
            setInterval(function () {
                if (document.activeElement && document.activeElement.tagName == "IFRAME") {
                    ...
                    if (!src || !src.match(/\?feedback/)) {
                        document.activeElement.blur();
                        window.focus();
                    }
                }
            }, 16);
            this.personalBest = parseInt(ig.storageGet('highscore')) | 0;
            if (window.Ejecta) {
                this.gameCenter = new Ejecta.GameCenter();
                this.gameCenter.authenticate();
                if (!ig.storageGet('removeAds')) {
            ...
            if (ig.doc) {
                var alpha = 'a-zßàáâãäåæçèéêëìíîïðñòóôõöøùúûüý';
                var num = '0-9';
                var punct = '!"#$%&\'()*+,\\-\\.\\/:;<=>?@\\[\\]\\\\\\^_`{|}~';
                var chars = alpha;
                if (this.textHasNumbers) {
                    chars += num;
                }
                if (this.textHasPunctuations) {
                    chars += punct;
                }
                this.reAllWordCharacter = new RegExp('^[' + chars + ']+$', 'i');
                this.reSplitNonWord = new RegExp('[^' + chars + ']', 'i');
                ig.doc.fastForwardScanAnimation();
            }
            this.bufferedLetters = [];
            var openFeedbackForm = document.getElementById('open-feedback-form');
            if (openFeedbackForm) {
                openFeedbackForm.addEventListener('click', this.openFeedbackForm.bind(this));
            }
        },
        reset: function () {
            this.entities = [];
        ...
        nextWave: function () {
            ...
            if (ig.doc) {
                // For custom text mode, increase difficulty over time
                console.log('=== WAVE ' + this.wave.wave + ' ===');
                console.log('Total fragments available:', ig.doc.fragments.length);
                console.log('Current fragment index:', this.wave.fragment);

                // Start with 5 sentences, add 2 more per wave
                var sentencesPerWave = 5 + (this.wave.wave - 1) * 2;
                for (var i = 0; i < sentencesPerWave; i++) {
                    this.nextDocFragment();
                }

                console.log('Wave spawn queue length:', this.wave.spawn.length);
                console.log('Wave spawn queue:', this.wave.spawn);
        ...
        nextDocFragment: function () {
            ...
            // Get the full text and split by line breaks
            var text = fragment.text.trim();
            var sentences = text.split(/\r?\n/);

            // Only take the FIRST non-empty sentence from this fragment
            for (var s = 0; s < sentences.length; s++) {
                var sentence = sentences[s].trim();

                // Clean up the sentence based on settings
                if (!this.textHasPunctuations) {
                ...
                // Only add if the sentence has content
                if (sentence.length > 0) {
                    // Determine ship type based on sentence length
                    var type = EntityEnemyMine;
                    if (sentence.length > 50) {
                        type = Math.random() > 0.5 ? EntityEnemyOppressor : EntityEnemyDestroyer;
                    } else if (sentence.length > 25) {
                        type = EntityEnemyDestroyer;
                    }

                    // Calculate wait time based on sentence length
                    var wait = (sentence.length / 10).limit(1, 5);
                    ...
                    // Only add ONE sentence per fragment call
                    break;
                }
            }
        },
        spawnCurrentWave: function () {
            if (!this.wave.spawn.length) {
                console.log('Spawn queue empty. Entities:', this.entities.length, 'WaveEndTimer:', !!this.waveEndTimer);
                if (this.entities.length <= 1 && !this.waveEndTimer) {
                    console.log('Starting wave end timer');
                    this.waveEndTimer = new ig.Timer(ig.doc ? 3 : 2);
                } else if (this.waveEndTimer && this.waveEndTimer.delta() > 0) {
                    console.log('Wave end timer complete, calling nextWave');
                    this.waveEndTimer = null;
                    this.nextWave();
                }
                ...
                // Check if canvas is empty (only player entity exists)
                var enemyCount = 0;
                for (var i = 0; i < this.entities.length; i++) {
                    if (this.entities[i] instanceof EntityEnemy) {
                        enemyCount++;
                    }
                }

                // If canvas is empty, spawn immediately regardless of word delay
                var canvasIsEmpty = enemyCount === 0;

                if (this.wordDelay > 0 && this.lastSpawnTimer && this.lastSpawnTimer.delta() < this.wordDelay && !canvasIsEmpty) {
                    this.spawnTimer.set(this.wave.currentSpawnWait * -1);
                    return;
                }
                this.lastSpawnTimer = new ig.Timer();
        ...
        registerTarget: function (letter, ent) {
            var c = this.translateUmlaut(this.prepareLetter(letter));
            if (this.targets[c] === undefined) {
                this.targets[c] = [ent];
            } else {
        ...
        prepareLetter: function (c) {
            if (this.textIsCaseSensitive) {
                return c;
            } else {
        ...
        keypress: function (ev) {
            if (ev.target.tagName == 'INPUT' || ev.ctrlKey || this.mode != ZType.MODE.GAME || this.menu) {
                return true;
            }
            var c = ev.charCode;
            if (c < 33) {
            ...
            ev.stopPropagation();
            ev.preventDefault();
            var letter = this.prepareLetter(String.fromCharCode(c));
            this.bufferLetter(letter);
            return false;
        },
        keydown: function (ev) {
        ...
        bufferLetter(letter) {
            this.bufferedLetters.push(letter);
        },
        shootBufferedLetters: function () {
            while (this.bufferedLetters.length > 0) {
                var letter = this.bufferedLetters.shift();
                if (!this.currentTarget) {
                    ...
                    if (potentialTargets && potentialTargets.length === 1) {
                    ...
                    } else if (potentialTargets && potentialTargets.length > 1) {
                        for (var i = 0; i < this.bufferedLetters.length; i++) {
                            var bufferedLetter = this.bufferedLetters[i];
                            var remainingPotentialTargets = [];
                            for (var j = 0; j < potentialTargets.length; j++) {
                                var enemy = potentialTargets[j];
                                if (enemy.remainingWord.charAt(i + 1) === bufferedLetter) {
                                    remainingPotentialTargets.push(enemy);
                                }
                            }
                            if (remainingPotentialTargets.length > 0) {
                                potentialTargets = remainingPotentialTargets;
                            } else {
                                break;
                            }
                        ...
                        for (var i = 0; i < potentialTargets.length; i++) {
                            ...
                            if (distance < nearestDistance || !bestTarget) {
            ...
            if (this.currentTarget) {
                ...
                if (hit) {
                    ...
                    if (target.dead) {
                        ...
                        var translated = this.translateUmlaut(this.prepareLetter(target.remainingWord.charAt(0)));
                        if (this.keyboard) {
        ...
        setGameOver: function () {
            if (this.score > this.personalBest) {
                ...
                ig.storageSet('highscore', this.personalBest);
            }
        ...
        showGameOverScreen: function () {
            ...
            if (this.adPage && !ig.storageGet('removeAds')) {
                this.adPage.show();
            }
        ...
        draw: function () {
            ...
            if (this.menu) {
                ...
                if (this.gameTransitionTimer) {
                    ...
                    if (this.menu.playerPos && this.player && this.player.pos) {
        ...
        openFeedbackForm: function (ev) {
            var close = document.createElement('div');
            close.classList.add('ztype-modal-close');
            close.addEventListener('click', this.closeFeedbackForm.bind(this));
            close.innerHTML = '&#215;';
            var iframe = document.createElement('iframe');
            iframe.classList.add('ztype-modal-frame');
            iframe.src = '/?feedback';
            var modal = document.createElement('div');
            modal.classList.add('ztype-modal');
            modal.appendChild(close);
            modal.appendChild(iframe);
            var modalContainer = document.createElement('div');
            modalContainer.classList.add('ztype-modal-container');
            modalContainer.appendChild(modal);
            this.modalFeedbackContainer = modalContainer;
            document.body.appendChild(modalContainer);
        },
        closeFeedbackForm: function (ev) {
            if (!this.modalFeedbackContainer) {
                return;
            }
            this.modalFeedbackContainer.remove();
            this.modalFeedbackContainer = null;
        }
    });
    ZType.MODE = {
        TITLE: 0,
    ...
    if (window.ZTypeDocumentMode) {
        var flags = parseInt(ig.queryParam('flags')) || window.ZTYPE_FLAGS;
        var enablePunctionations = (flags & ZType.FLAG.PUNCTUATION);
        var enableNumbers = (flags & ZType.FLAG.NUMBERS);

        console.log('ZTypeCustomFragments exists?', typeof window.ZTypeCustomFragments);
        console.log('ZTypeCustomFragments value:', window.ZTypeCustomFragments);

        // Check if custom fragments are provided directly
        if (window.ZTypeCustomFragments && window.ZTypeCustomFragments.length > 0) {
            console.log('=== USING CUSTOM FRAGMENTS ===');
            console.log('Total custom fragments:', window.ZTypeCustomFragments.length);
            // Create a mock document scanner with pre-made fragments
            ig.doc = {
                fragments: [],
                looksLikeEnglish: false,
            ...
            // Create fragments from the custom array
            for (var i = 0; i < window.ZTypeCustomFragments.length; i++) {
                ig.doc.fragments.push({
                    text: window.ZTypeCustomFragments[i],
                    element: null
                });
                console.log('Custom Fragment ' + i + ':', window.ZTypeCustomFragments[i]);
            }
        } else {
            ig.doc = new ig.DocumentScanner(document.body, enableNumbers, enablePunctionations);
            if (!ig.ua.mobile) {
